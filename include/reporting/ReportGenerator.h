...
class ReportGenerator {
 public:
  ...
  // -------------------------------------------------------------------
  void RecordEquitySnapshot(uint64_t timestamp, int64_t equity, int64_t cash, int64_t realized_pnl,
                            int64_t unrealized_pnl, int64_t drawdown, bool has_open_position);
 ...
 private:
  ...
  PerformanceSummary ComputeSummaryFromTrades(const std::vector<TradeRecord>& trades,
                                              money_t initial_capital, money_t final_equity) const;
  ...
  void WritePerStrategyBreakdownCsv(
