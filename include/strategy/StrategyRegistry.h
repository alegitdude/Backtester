...
using StrategyFactoryFunc =
    std::function<std::unique_ptr<IStrategy>(const std::string, const IMarketDataProvider&)>;
...
#define REGISTER_STRATEGY(ClassName, StringName)                                                   \
  ...
  static bool const registered_##ClassName = []() {                                                \
    backtester::StrategyRegistry::Register(                                                        \
        StringName, [](const std::string strategy_id, const backtester::IMarketDataProvider& md) { \
