...
class MarketStateManager : public IMarketDataProvider {
 public:
  ...
  const std::vector<BidAskPair> GetOBSnapshotByPub(uint32_t instrument_id, uint16_t publisher_id,
  ...
  inline const MarketSnapshot* GetSnapshotByInstr(uint32_t instr_id) const override {
    ...
    if (BT_UNLIKELY(it == snapshots_.end())) {
      throw std::runtime_error(
          fmt::format("GetSnapshotByInstr in market state "
                      "tried to find an unknown instrument: {}",
                      instr_id));
    }
    return it->second;
  }
