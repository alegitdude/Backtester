...
inline TimeOfDay GetTimeOfDay(uint64_t epoch_nanos) {
  ...
  return {static_cast<uint8_t>(day_seconds / 3600),
          static_cast<uint8_t>((day_seconds % 3600) / 60)};
}

std::string EpochToString(uint64_t epoch_nanos, const std::string& time_zone = "UTC");
timestamp_t GetTimezoneOffset(const std::string& timezone);
TimeParseResult ParseIsoToUnix(std::string_view s);

inline int fast_atoi_2(const char* s) { return (s[0] - '0') * 10 + (s[1] - '0'); }
