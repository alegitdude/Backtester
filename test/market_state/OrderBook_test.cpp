...
TEST_F(OrderBookTest, ES20251105_FullDay_MatchesDB_MBP10_OnePub) {
  DataSourceConfig data_source = {"ES",
                                  1,
                                  {},
  ...
  if (use_sample)
    data_source.data_filepath = kTestDataFolder / "ES-glbx-20251105_500k_rows.csv.zst";
  ...
  if (use_sample) {
    LoadExpectedMbp10(kTestDataFolder / "ES-glbx-20251105.mbp-10-sample.csv.zst");
  } else {
  ...
  for (const DataSourceConfig& source : config.data_configs) {
    if (data_reader_manager.LoadNextEventFromSource(source.data_source_id, mbo))
