...
ExecutionHandler::ExecutionHandler(EventQueue& event_queue, const uint64_t latency_ms)
    : event_queue_(event_queue),
      latency_ns_(latency_ms * 1'000'000ULL),
...
void ExecutionHandler::CheckFillsQueuePosition(const MarketByOrderEvent& mbo_event) {
    ...
    for (auto& [order_id, pending] : pending_orders_) {
        ...
        bool same_price = (mbo_event.price == pending.price);

        // Determine if this market event is on the same side of the book
        // as our resting order. Our bid rests on the bid side; a trade that
        // lifts the bid is a sell aggressor hitting our side.
        ...
        // Trade at our price level: drains queue AND can fill us
        if (mbo_event.type == EventType::kMarketTrade ||
            mbo_event.type == EventType::kMarketFill) {
