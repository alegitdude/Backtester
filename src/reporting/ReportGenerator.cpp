...
ReportGenerator::ReportGenerator(const AppConfig& config)
    : config_(config), peak_equity_(config.initial_cash) {
...
void ReportGenerator::RecordEquitySnapshot(uint64_t timestamp, int64_t equity, int64_t cash,
                                           int64_t realized_pnl, int64_t unrealized_pnl,
                                           int64_t drawdown, bool has_open_position) {
  ...
  equity_curve_.push_back(
      {timestamp, equity, cash, realized_pnl, unrealized_pnl, drawdown, has_open_position});
}
...
void ReportGenerator::GenerateReport(const PortfolioManager& portfolio,
  ...
  if (breakdowns.size() > 1) {
    WritePerStrategyBreakdownCsv(output_dir + "/strategy_breakdown.csv", breakdowns);
  }

  spdlog::info("ReportGenerator: Reports written to {}", output_dir);
  spdlog::info(
      "ReportGenerator: Total PnL={} | Return={:.2f}% | "
      "Sharpe={:.3f} | MaxDD={:.2f}% | Trades={}",
      FormatScaledPrice(summary.total_pnl), summary.total_return_pct * 100.0, summary.sharpe_ratio,
      summary.max_drawdown_pct * 100.0, summary.total_trades);
}
...
PerformanceSummary ReportGenerator::ComputeSummary(const PortfolioManager& portfolio) const {
  ...
  PerformanceSummary summary = ComputeSummaryFromTrades(trades, config_.initial_cash, final_equity);
  ...
  summary.max_drawdown_pct =
      (curve_peak > 0) ? static_cast<double>(max_dd) / static_cast<double>(curve_peak) : 0.0;

  // CAGR
  double duration_years = summary.backtest_duration_days / 252.0;
  if (duration_years > 0.0) {
    double equity_ratio =
        static_cast<double>(final_equity) / static_cast<double>(config_.initial_cash);
    if (equity_ratio > 0.0) {
      summary.annualized_return_pct = std::pow(equity_ratio, 1.0 / duration_years) - 1.0;
    }
  }
  ...
  if (summary.annualized_volatility > 0.0) {
    summary.sharpe_ratio =
        (summary.annualized_return_pct - config_.risk_free_rate) / summary.annualized_volatility;
  }

  // Sortino: annualized_return / downside_deviation
  if (summary.downside_deviation > 0.0) {
    summary.sortino_ratio =
        (summary.annualized_return_pct - config_.risk_free_rate) / summary.downside_deviation;
  }

  // Calmar: annualized_return / max_drawdown_pct
  if (summary.max_drawdown_pct > 0.0) {
    summary.calmar_ratio = summary.annualized_return_pct / summary.max_drawdown_pct;
  }

  // Time in market from equity curve
  if (!equity_curve_.empty()) {
    uint64_t total_time = equity_curve_.back().timestamp - equity_curve_.front().timestamp;
    uint64_t time_with_position = 0;

    for (size_t i = 1; i < equity_curve_.size(); ++i) {
      if (equity_curve_[i - 1].has_open_position == true) {
        time_with_position += equity_curve_[i].timestamp - equity_curve_[i - 1].timestamp;
      }
    }

    summary.time_in_market_pct =
        (total_time > 0) ? static_cast<double>(time_with_position) / static_cast<double>(total_time)
                         : 0.0;
  }
...
PerformanceSummary ReportGenerator::ComputeSummaryFromTrades(const std::vector<TradeRecord>& trades,
  ...
  if (s.avg_loss != 0) {
    s.avg_win_loss_ratio =
        static_cast<double>(s.avg_win) / static_cast<double>(std::abs(s.avg_loss));
  }

  // Profit factor
  if (gross_loss > 0) {
    s.profit_factor = static_cast<double>(gross_profit) / static_cast<double>(gross_loss);
  ...
  if (s.total_trades > 0) {
    s.avg_pnl_per_trade = s.total_realized_pnl / static_cast<int64_t>(s.total_trades);
  }

  // Average trade duration (rough: total span / num trades)
  if (s.total_trades > 1) {
    s.avg_trade_duration_ns =
        static_cast<double>(last_trade_ts - first_trade_ts) / static_cast<double>(s.total_trades);
  }
...
std::unordered_map<std::string, PerformanceSummary> ReportGenerator::ComputePerStrategySummary(
  ...
  for (auto& [strategy_id, trades] : by_strategy) {
    ...
    result[strategy_id] = ComputeSummaryFromTrades(trades, config_.initial_cash, strat_equity);
  }
...
double ReportGenerator::ComputeAnnualizedVolatility() const {
  ...
  double total_ns =
      static_cast<double>(equity_curve_.back().timestamp - equity_curve_.front().timestamp);
  double obs_per_year =
      (total_ns > 0.0) ? static_cast<double>(returns.size()) * (kNanosPerYear / total_ns) : 0.0;
...
void ReportGenerator::WriteTradeLogCsv(const std::string& filepath,
  ...
  file.close();
  spdlog::info("ReportGenerator: Trade log written to {} ({} trades)", filepath, trades.size());
}
...
void ReportGenerator::WriteEquityCurveCsv(const std::string& filepath) const {
  ...
  for (const auto& snap : equity_curve_) {
    file << snap.timestamp << "," << FormatScaledPrice(snap.equity) << ","
         << FormatScaledPrice(snap.cash) << "," << FormatScaledPrice(snap.realized_pnl) << ","
         << FormatScaledPrice(snap.unrealized_pnl) << "," << FormatScaledPrice(snap.drawdown) << ","
         << snap.has_open_position << "\n";
  }

  file.close();
  spdlog::info("ReportGenerator: Equity curve written to {} ({} snapshots)", filepath,
               equity_curve_.size());
}

void ReportGenerator::WritePerStrategyBreakdownCsv(
  ...
  for (const auto& [strategy_id, s] : breakdowns) {
    file << strategy_id << "," << FormatScaledPrice(s.total_pnl) << ","
         << s.total_return_pct * 100.0 << "," << s.total_trades << "," << s.winning_trades << ","
         << s.losing_trades << "," << s.win_rate_pct * 100.0 << "," << s.profit_factor << ","
         << FormatScaledPrice(s.avg_win) << "," << FormatScaledPrice(s.avg_loss) << ","
         << FormatScaledPrice(s.largest_win) << "," << FormatScaledPrice(s.largest_loss) << ","
         << FormatScaledPrice(s.avg_pnl_per_trade) << "," << s.max_position_held << ","
         << s.total_volume_traded << "," << FormatScaledPrice(s.total_commissions) << "\n";
  }

  file.close();
  spdlog::info("ReportGenerator: Strategy breakdown written to {} ({} strategies)", filepath,
               breakdowns.size());
}
