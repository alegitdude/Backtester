...
TimeParseResult ParseIsoToUnix(std::string_view s) {
  ...
  if (s.length() < 19) {
    return IsoFail(s, fmt::format("timestamp too short: need at least 19 chars"
                                  "'YYYY-MM-DDTHH:MM:SS', got {}",
                                  s.length()));
  }

  if (s[4] != '-')
    return IsoFail(s, 4, fmt::format("expected '-' between year and month, found '{}'", s[4]));
  if (s[7] != '-')
    return IsoFail(s, 7, fmt::format("expected '-' between month and day, found '{}'", s[7]));
  if (s[10] != 'T' && s[10] != ' ')
    return IsoFail(s, 10,
                   fmt::format("expected 'T' or ' ' between date and time, found '{}'", s[10]));
  if (s[13] != ':')
    return IsoFail(s, 13, fmt::format("expected ':' between hour and minute, found '{}'", s[13]));
  if (s[16] != ':')
    return IsoFail(s, 16, fmt::format("expected ':' between minute and second, found '{}'", s[16]));
  ...
  auto field = [&](size_t begin, size_t width, int& out,
    ...
    if (res.ec != std::errc{} || res.ptr != data + begin + width) {
      return IsoFail(s, begin,
                     fmt::format("{} must be {} digits, found \"{}\"", name, width,
                                 std::string(s.substr(begin, width))));
    }
    return std::nullopt;
  };
  ...
  if (t.tm_mon < 0 || t.tm_mon > 11)
    return IsoFail(s, 5, fmt::format("month out of range: {} (expected 01-12)", t.tm_mon + 1));
  if (t.tm_mday < 1 || t.tm_mday > 31)
    return IsoFail(s, 8, fmt::format("day out of range: {} (expected 01-31)", t.tm_mday));
  if (t.tm_hour < 0 || t.tm_hour > 23)
    return IsoFail(s, 11, fmt::format("hour out of range: {} (expected 00-23)", t.tm_hour));
  if (t.tm_min < 0 || t.tm_min > 59)
    return IsoFail(s, 14, fmt::format("minute out of range: {} (expected 00-59)", t.tm_min));
  if (t.tm_sec < 0 || t.tm_sec > 59)
    return IsoFail(s, 17, fmt::format("second out of range: {} (expected 00-59)", t.tm_sec));
  ...
  if (pos < s.length() && s[pos] == '.') {
    ...
    if (res.ec != std::errc{}) {
      return IsoFail(s, pos + 1,
                     "fractional seconds after '.'"
                     "must be 1-9 digits");
    }
    size_t digits = static_cast<size_t>(res.ptr - nano_start);
    if (digits > 9) {
      return IsoFail(s, pos + 1, fmt::format("too many fractional digits: {} (max 9)", digits));
    }
    nanos = raw_val;
  ...
  if (pos < s.length()) {
    if (s[pos] == 'Z' && pos + 1 == s.length()) {
      ...
      return IsoFail(
          s, pos,
          fmt::format("unexpected trailing characters: \"{}\"", std::string(s.substr(pos))));
    }
  }
...
std::string EpochToString(uint64_t epoch_nanos, const std::string& timezone) {
  ...
  oss << std::setfill('0') << std::setw(4) << (tm_info.tm_year + 1900) << "-" << std::setw(2)
      << (tm_info.tm_mon + 1) << "-" << std::setw(2) << tm_info.tm_mday << " " << std::setw(2)
      << tm_info.tm_hour << ":" << std::setw(2) << tm_info.tm_min << ":" << std::setw(2)
      << tm_info.tm_sec << "." << std::setw(9) << nanos;
  return oss.str();
};
