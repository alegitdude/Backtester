...
constexpr double kDefaultStockClearingFee = 0.0002;
constexpr double kDefaultStockOrderMin = 0.35;
constexpr double kDefaultStockPerShare = 0.035;
constexpr double kDefaultRiskFreeRate = 0.05;

constexpr RiskLimits kDefaultRiskLimits = {
    RiskMode::PercentOfAcct, kDefaultMaxPositionSize, 20'000'000, 0, 400'000'000, 0};
}  // namespace
...
void ResolvePath(std::string& path, const std::filesystem::path config_dir) {
  std::filesystem::path fp(path);
  if (fp.is_relative()) {
    path = (config_dir / fp).lexically_normal().string();
  }
}
...
AppConfig ParseConfigFromJson(const nlohmann::json& data, std::filesystem::path config_path) {
  ...
  auto start_result = time::ParseIsoToUnix(start_str);
  if (!start_result.success) {
    throw std::runtime_error(
        fmt::format("Config 'start_time' error: {} in string ", start_result.error_msg));
  }
  ...
  if (!end_result.success) {
    throw std::runtime_error(
        fmt::format("Config 'end_time' error: {} in string ", end_result.error_msg));
  }
  config.end_time = end_result.unix_nanos;

  if (config.start_time >= config.end_time) {
    throw std::runtime_error(fmt::format(
        "Config start_time {} is equal to or greater than end_time {}. Backtest can't run.",
        config.start_time, config.end_time));
  }

  // MARK: Execution Latency
  config.execution_latency_ms =
      GetOptional<uint64_t>(data, "execution_latency_ms", "Global Settings")
          .value_or(kDefaultExecLatencyMs);

  // MARK: Snapshot Interval
  auto interval_ms = GetOptional<uint64_t>(data, "snapshot_interval_ms", "Global Settings")
                         .value_or(kDefaultSnapshotIntervalMs);
  ...
  // MARK: Initial Cash
  config.initial_cash =
      GetOptional<money_t>(data, "initial_cash", "Global Settings").value_or(kDefaultInitialCash);
  ...
  config.log_file_path =
      GetOptional<std::string>(data, "log_file_path", "Global Settings").value_or("");
  if (config.log_file_path == "") config.log_file_path = kRootFolder / "logs";
  ResolvePath(config.log_file_path, config_dir);

  // MARK: Report Output Directory
  config.report_output_dir =
      GetOptional<std::string>(data, "report_output_dir", "Global Settings").value_or("");
  if (config.report_output_dir == "") config.report_output_dir = kDefaultReportDir;
  ResolvePath(config.report_output_dir, config_dir);

  // MARK: Risk-Free Rate
  config.risk_free_rate =
      GetOptional<double>(data, "risk_free_rate", "Global Settings").value_or(kDefaultRiskFreeRate);
  if (config.risk_free_rate > 1.0) {
    spdlog::error(R"(Config 'risk_free_rate' {} above 1.0 or needs to be expressed)
				as a decimal fraction, not a percentage)",
                  config.risk_free_rate);
    throw std::runtime_error(
        fmt::format("Config 'risk_free_rate' = {} is implausible. Must be under 1.0 and"
                    "expressed as a decimal fraction (e.g. 0.05 for 5%), not a percentage.",
                    config.risk_free_rate));
  }
  ...
  // Mark: strategy intrument check in traded instruments
  for (auto& strat : config.strategies) {
    ...
    if (!instr_exists)
      throw std::runtime_error(fmt::format(R"(Instrument {} in strategy {} 
				not found in traded instruments. Unable to use strategy)",
                                           strat.traded_instr_id, strat.name));
  }
  ...
  for (const auto& stream : data["data_streams"]) {
    ...
    ResolvePath(tmp_sym_path, config_dir);
    ...
    ResolvePath(data_config.data_filepath, config_dir);
    ...
    data_config.compression =
        StrToCompression(GetRequired<std::string>(stream, "compression", context));
    data_config.price_format =
        StrToPriceFormat(GetRequired<std::string>(stream, "price_format", context));
    data_config.ts_format =
        StrToTSFormat(GetRequired<std::string>(stream, "timestamp_format", context));
...
std::vector<Symbol> ParseDataSymbols(const std::string& filepath) {
  ...
  while (std::getline(file, line)) {
    ...
    if (std::getline(ss, symbol, ',') && std::getline(ss, instrument, ',') &&
        std::getline(ss, date)) {
      try {
        row.symbol = symbol;
...
std::vector<TradedInstrument> ParseTradedInstrs(const nlohmann::json& data) {
  ...
  for (const auto& item : data) {
    ...
    instr.instrument_id = GetRequired<uint32_t>(item, "instrument_id", "Traded Instruments");
    instr.instrument_type =
        ParseInstrType(GetRequired<std::string>(item, "instrument_type", "Traded Instruments"));
    instr.tick_size = numericUtils::DoubleToFixedPoint(
        GetRequired<double>(item, "tick_size", "Traded Instruments"));
    if (instr.tick_size == 0) {
      throw std::runtime_error(
          fmt::format("Tick size cannot be 0, error for instrument {}", instr.instrument_id));
    }
    instr.tick_value = numericUtils::DoubleToFixedPoint(
        GetRequired<double>(item, "tick_value", "Traded Instruments"));
    if (instr.tick_value == 0) {
      throw std::runtime_error(
          fmt::format("Tick value cannot be 0, error for instrument {}", instr.instrument_id));
    }
    instr.init_margin_req = numericUtils::DoubleToFixedPoint(
        GetRequired<double>(item, "init_margin_req", "Traded Instruments"));
    instr.maint_margin_req = numericUtils::DoubleToFixedPoint(
        GetRequired<double>(item, "main_margin_req", "Traded Instruments"));
...
RiskLimits ParseRiskLimits(const nlohmann::json& data) {
  ...
  res.risk_mode = ParseRiskMode(GetOptional<std::string>(data, "risk_mode", context).value_or(""));
  res.max_position_size =
      GetOptional<int64_t>(data, "max_position_size", context).value_or(kDefaultMaxPositionSize);
  res.max_risk_per_trade_pct =
      numericUtils::DoubleToFixedPoint(GetOptional<double>(data, "max_risk_per_trade_pct", context)
                                           .value_or(kDefaultMaxRiskPerTradePct));
  res.max_portfolio_delta =
      numericUtils::DoubleToFixedPoint(GetOptional<double>(data, "max_portfolio_delta", context)
                                           .value_or(kDefaultMaxPortfolioDelta));
  res.max_drawdown_pct = numericUtils::DoubleToFixedPoint(
      GetOptional<double>(data, "max_drawdown_pct", context).value_or(kDefaultMaxDrawdownPct));
  res.max_delta_per_trade = numericUtils::DoubleToFixedPoint(
      GetOptional<double>(data, "max_delta_per_trade", context).value_or(kDefaultMaxDeltaPerTrade));
...
CommissionStruct ParseCommissions(const nlohmann::json& data) {
  ...
  res.fut_per_contract = numericUtils::DoubleToFixedPoint(
      GetOptional<double>(data, "fut_per_contract", context).value_or(kDefaultFutPerContract));
  res.stock_clearing_fee = numericUtils::DoubleToFixedPoint(
      GetOptional<double>(data, "stock_clearing_fee", context).value_or(kDefaultStockClearingFee));
  res.stock_order_min = numericUtils::DoubleToFixedPoint(
      GetOptional<double>(data, "stock_order_min", context).value_or(kDefaultStockOrderMin));
  res.stock_per_share = numericUtils::DoubleToFixedPoint(
      GetOptional<double>(data, "stock_per_share", context).value_or(kDefaultStockPerShare));
...
T GetRequired(const nlohmann::json& j, const std::string& key, const std::string& context) {
  ...
  if (val.is_number() && val.get<double>() < 0) {
    throw std::runtime_error(
        fmt::format("Config Error: Negative value {} assigned to unsigned key '{}' in {}",
  ...
  try {
    ...
    throw std::runtime_error(
        fmt::format("Config Error: Invalid type for '{}' in {}. Expected {}, but found {}.", key,
                    context, typeid(T).name(), val.type_name()));
  }
}
...
std::optional<T> GetOptional(const nlohmann::json& j, const std::string& key,
  ...
  if (!j.contains(key) || j.at(key).is_null()) {
    spdlog::warn("Config: Key '{}' in {} is null. Using default.", key, context);
    return std::nullopt;
  }
